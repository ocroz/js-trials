...
export class IssueService {
  ...
  getIssue(key: string): Issue {
    return this.store.getState().issues.find(issue => issue.key === key);
  }
  ...
  postIssue(issue: Issue): void {
    ...
    this.http.post<any>(`${this.jiraApi}/issue`, issue, { headers, observe })
      // .pipe(
      //   catchError(this.handleError('postIssue')),
      //   tap(issue => issue && this.dataActions.receiveOK(issue)), // Remove observe in httpOptions above
      // )
